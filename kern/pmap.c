...
i386_vm_init(void)
{
    ...
    // Remove this line when you're ready test this function.
    ...
    //之上备用
    size_t page_size;
    //ori: page_size = ROUNDUP((sizeof(struct Page) * npage), PGSIZE);
    page_size = (sizeof(struct Page) * npage);
    ...
    // - the read-only version mapped at UPAGES -- kernel R, user R
    // Your code goes here:
    boot_map_segment(pgdir, UPAGES, page_size, PADDR(pages), PTE_U | PTE_P);
    //by landq
    ...
    // - envs itself -- kernel RW, user NONE
    // - the image of envs mapped at UENVS -- kernel R, user R
    boot_map_segment(pgdir, UENVS, env_size, PADDR(envs), PTE_U | PTE_P);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if(!(*pgdir & PTE_P)){
    	...
    	*pgdir = page2pa(page_tmp) | PTE_U | PTE_W | PTE_P;
        //这个地方为什么没有直接return page_tmp呢，因为需要加上一些页面属性
    }
